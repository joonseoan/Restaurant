...
class GuestbookAllPosted extends Component {
        ...
        return (
          <div key={guestbook._id}>
            <div>
              ...
              xxx@Owl Korean Restaurant in {guestbook.city}
            </div>
            <div>
              <i>{guestbook.visitedAt}</i>
            </div>
  ...
  render() {
    return (
      <div>
        <div>
          <h3>Customer's Best Choices</h3>

          <h5>(Please, click on the list)</h5>
        </div>

        <div>
          <ul>{this.renderGuestBooks()}</ul>
        </div>

        <div>
          <Link to="/">Back to main page</Link>

          <Link to="/emailPasswordInput">Delete your post</Link>
        </div>
      </div>
    );
  }
